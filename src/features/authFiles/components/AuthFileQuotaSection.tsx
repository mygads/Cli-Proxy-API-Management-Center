...
// Map a provider type to its quota config. Kiro and GitHub are first-class
// — they have BE endpoints under /v0/management/{kiro,github}-quota, NOT
// the api-call proxy that gemini/claude/codex use, so they need their own
// config (otherwise the fallback below sends a Gemini probe and surfaces
// "missing project ID" on Kiro/GitHub cards).
const getQuotaConfig = (type: QuotaProviderType) => {
  if (type === 'antigravity') return ANTIGRAVITY_CONFIG;
  if (type === 'claude') return CLAUDE_CONFIG;
  if (type === 'codex') return CODEX_CONFIG;
  if (type === 'kimi') return KIMI_CONFIG;
  if (type === 'github') return GITHUB_CONFIG;
...
export function AuthFileQuotaSection(props: AuthFileQuotaSectionProps) {
  ...
  const quota = useQuotaStore((state) => {
    ...
    if (quotaType === 'github') return state.githubQuota[file.name] as QuotaState;
  ...
  const updateQuotaState = useQuotaStore((state) => {
    ...
    if (quotaType === 'github') return state.setGithubQuota as unknown as (updater: unknown) => void;
